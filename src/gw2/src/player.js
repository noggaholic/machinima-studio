...
module.exports = (process, module, memory) => {
  ...
  const playerCurrPosition = {
    x: null,
    y: null,
    z: null
  };

  const mouseRealPosition = {
    x: null,
    y: null,
    z: null
  };

  const movementOriginalByteCode = Buffer.alloc(offsets.player.movement.byteLength);
  const movementOffset = module + Number(ptrs.player.movement.original);
  const movementBufferLength = offsets.player.movement.byteLength;
  memory.readData(movementOffset, movementOriginalByteCode, offsets.player.movement.byteLength);

  const debugData = {};
  const debugStr = Buffer.alloc(0x100);
  const mapGUIDStr = Buffer.alloc(0x48);
  const MapFloor = Buffer.alloc(0x4);
  const MapId = Buffer.alloc(0x4);
  that.getDebugInfo = () => {
  ...
  that.getRotation = () => {
    ...
    const cos = playerRotation.readFloatLE(0);
    const sin = playerRotation.readFloatLE(0x4);
    let result = Math.atan2(cos, sin) * (180 / Math.PI);
    if (result < 0) result += 360;
    return {
      degrees: parseInt(result),
      headingA: cos,
      headingB: sin,
      radians: Math.atan2(cos, sin)
    };
  };
