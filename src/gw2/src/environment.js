...
module.exports = (process, module, memory) => {
  ...
  const fogColorBase = memory.readMultiLevelPtr(offsets.environment.fog.offset);
  const fogColor = Buffer.alloc(0xC);
  const fogDensity = Buffer.alloc(0x4);
  const fogBlock = Buffer.alloc(0x4);
  const timeOfDay = Buffer.alloc(0x4);
  const timeOfDayRead = Buffer.alloc(0x4);

  offsets.camera.offset[0] = ptrs.camera.offset;
  const cameraOffsetBase = memory.readMultiLevelPtr(offsets.camera.offset);
  ...
  that.enableEnvPointer = () => {
    const cameraNumber = (cameraOffsetBase + offsets.camera.envPointerFix);
    const envfix = Buffer.from([0x89, 0x35, 0xFC, 0x1E, 0x7B, 0x2F, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90]);
  ...
  that.disableEnvPointer = () => {
    ...
    const envfix = Buffer.from([0x0F, 0x2F, 0xC1, 0x72, 0x0D, 0xF3, 0x0F, 0x11, 0x8E, 0xCC, 0x19, 0x00, 0x00]);
    memory.writeData(module + ptrs.environment.enableTimeOfDay, envfix, envfix.byteLength);
  };

  that.setTimeOfDay = (value) => {
    let cameraEnvPTR = (cameraOffsetBase + offsets.camera.envPointerFix);
  ...
  const savePattern = (reference, section, base) => {
