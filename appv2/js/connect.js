...
$('a[href="environment-colors.html"]').on('click', function(event) {
  ...
  $(".fogDensity").on("change", function() {
    console.log('"input change"');
    console.log('"input fogDensity"');
    console.log('"input change"');

  });

  $('.fogDensity').on( "slidechange", function( event, ui ) {
    socket.emit('ENV_SET_FOG_DENSITY', ui.value);
  });
