...
module.exports = (process, module, memory, window, player, sendMessage) => {
  ...
  const playerPosition          = new Buffer(0xC);
  ...
  const spectate = Spectate(process, module, memory, that);
  let pitchDown = 0, pitchUp = 0, yawRight = 0, yawLeft = 0, rollRight = 0, rollLeft = 0;
  ...
  that.enableCameraControls = () => {
    ...
    cameraControls = setInterval(() => {
      ...
      let curFwd    = that.getFwdPosition();
      let cameraPos = that.getPosition();
      let mousePos  = player.getMousePosition();
      let rad       = player.getRotation();
      let rotation  = that.getRotation();

      if( checkForKeyStroke(robot.KEY_F3) ) {
        sendMessage('CAMERA_ADD_POSITION', { pos: cameraPos, lookAt: curFwd, timeOfDay: environment.getTimeOfDay() });
      }
      ...
      hyp = Math.sqrt(dir.x*dir.x + dir.y*dir.y + dir.z*dir.z);
      dir.x /= hyp;
      dir.y /= hyp;
      dir.z /= hyp;
